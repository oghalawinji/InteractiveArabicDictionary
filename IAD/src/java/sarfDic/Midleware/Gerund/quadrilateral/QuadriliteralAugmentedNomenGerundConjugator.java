...
public class QuadriliteralAugmentedNomenGerundConjugator {
    ...
    private QuadriliteralAugmentedNomenGerundConjugator() {
        for (int i = 1; i <= 3; i++) {
            String gerundClassName = /*getClass().getPackage().getName()*/"sarf.gerund.quadrilateral.augmented.nomen" + ".pattern." + "GerundPattern" + i;
            try {
    ...
    public List createGerundList(AugmentedQuadriliteralRoot root, int formulaNo) {
        String gerundPatternClassName = /*getClass().getPackage().getName() */"sarf.gerund.quadrilateral.augmented.nomen"+ ".pattern." + "GerundPattern" + formulaNo;
        List gerundDisplayList = createEmptyList();

        sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund gerundInstance = (sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund) gerundClassMap.get(formulaNo + "");
        ...
        for (int i = 0; i < indexArray.length; i++) {
            ...
            try {
                sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund gerund = (sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund) Class.forName(gerundPatternClassName).getConstructors()[1].newInstance(parameters);
    ...
    public Map createGerundMap(AugmentedQuadriliteralRoot root) {
        ...
        while (iter.hasNext()) {
            ...
            sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund gerundInstance = (sarf.gerund.quadrilateral.augmented.nomen.QuadriliteralAugmentedNomenGerund) gerundClassMap.get(augmentationFormula.getFormulaNo() + "");
