 ...
 * @author Gowzancha
 */
import sarf.gerund.quadrilateral.augmented.*;
...
public class QuadriliteralAugmentedGerundConjugator {
    ...
    private QuadriliteralAugmentedGerundConjugator() {
        for (int i = 1; i <= 3; i++) {
            String gerundClassName = /*getClass().getPackage().getName()*/"sarf.gerund.quadrilateral.augmented" + ".pattern." + "GerundPattern" + i;
            try {
    ...
    public List createGerundList(AugmentedQuadriliteralRoot root, int formulaNo) {
        String gerundPatternClassName = /*getClass().getPackage().getName()*/ "sarf.gerund.quadrilateral.augmented"+ ".pattern." + "GerundPattern" + formulaNo;
        ...
        for (int i = 0; i < indexArray.length; i++) {
            ...
            try {
                QuadriliteralAugmentedGerund gerund = (QuadriliteralAugmentedGerund) Class.forName(gerundPatternClassName).getConstructors()[1].newInstance(parameters);
