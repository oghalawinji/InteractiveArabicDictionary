...
public class TrilateralAugmentedNomenGerundConjugator {
    ...
    private TrilateralAugmentedNomenGerundConjugator() {
        for (int i = 1; i <= 12; i++) {
            String gerundClassName = /*getClass().getPackage().getName()*/"sarf.gerund.trilateral.augmented.nomen" + ".pattern." + "GerundPattern" + i;
            try {
    ...
    public List createGerundList(AugmentedTrilateralRoot root, int formulaNo) {
        String gerundPatternClassName = /*getClass().getPackage().getName()*/"sarf.gerund.trilateral.augmented.nomen" + ".pattern." + "GerundPattern" + formulaNo;
        ...
        for (int i = 0; i < indexArray.length; i++) {
            ...
            try {
                TrilateralAugmentedNomenGerund gerund = (TrilateralAugmentedNomenGerund) Class.forName(gerundPatternClassName).getConstructors()[1].newInstance(parameters);
