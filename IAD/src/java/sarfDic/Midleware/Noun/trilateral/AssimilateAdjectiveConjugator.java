...
public class AssimilateAdjectiveConjugator implements IUnaugmentedTrilateralNounConjugator{
    ...
    private void loadFormulaName(String formulaID) {
        String formulaClassName = /*getClass().getPackage().getName()*/"sarf.noun.trilateral.unaugmented.assimilate" + ".nonstandard.NounFormula" + formulaID;
        try {
    ...
    public List getAppliedFormulaList(UnaugmentedTrilateralRoot root) {
        AssimilateAdjectiveFormulaTree formulaTree = sarfDic.Midleware.DatabaseManager.getInstance().getAssimilateAdjectiveFormulaTree(root.getC1());
        if (formulaTree == null) {
            return null;
        }
