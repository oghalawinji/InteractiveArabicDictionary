...
public class NonStandardInstrumentalConjugator implements IUnaugmentedTrilateralNounConjugator {
    ...
    private NonStandardInstrumentalConjugator() {
        for (int i = 1; i <= 15; i++) {
            String formulaClassName = /*getClass().getPackage().getName() */"sarf.noun.trilateral.unaugmented.instrumental"+ ".nonstandard.NounFormula" + i;
            try {
    ...
    public List getAppliedFormulaList(UnaugmentedTrilateralRoot root) {
        ...
        XmlNonStandardInstrumentalNounFormulaTree formulaTree = sarfDic.Midleware.DatabaseManager.getInstance().getInstrumentalNounFormulaTree(root.getC1());
        if (formulaTree == null) {
            return null;
        }
