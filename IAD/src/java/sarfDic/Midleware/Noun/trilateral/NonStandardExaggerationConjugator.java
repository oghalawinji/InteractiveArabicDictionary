...
public class NonStandardExaggerationConjugator implements IUnaugmentedTrilateralNounConjugator{
    ...
    private NonStandardExaggerationConjugator() {
        for (int i=1; i<=10;i++) {
            String formulaClassName = /*getClass().getPackage().getName()*/"sarf.noun.trilateral.unaugmented.exaggeration"+".nonstandard.NounFormula"+i;
            try {
    ...
    public List getAppliedFormulaList(UnaugmentedTrilateralRoot root) {
        XmExaggerationNounFormulaTree formulaTree =  sarfDic.Midleware.DatabaseManager.getInstance().getExaggerationNounFormulaTree(root.getC1());
        if (formulaTree == null)
            return null;
